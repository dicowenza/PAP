...
calc_t valeur_absolue(calc_t x){
  if(x<0)
    return ((-1) * x);
  else
    return x;
		  ...
		  calc_t radius, unsigned natoms, unsigned offset)
{
  ...
  unsigned axe= index / offset;
  if(pos[index]<=(min[axe]+radius))

    speed[index]= (1) * valeur_absolue(speed[index]);

  else if (pos[index]>=(max[axe]-radius))
    speed[index]= (-1) * valeur_absolue(speed[index]);
}
