...
{
  unsigned res = 0;
  int dep [GRAIN][GRAIN];/////[GRAIN][GRAIN + 1]
...
#pragma omp single
    ...
    for (int j = 0; j < GRAIN; j++)
      for (int i = 0; i < GRAIN; i++)
        #pragma omp task firstprivate(i,j) shared(change) depend(out:dep[i][j]) depend(in: dep[i-1][j],dep[i][j-1])
        if(change |= tile_down_right (j * TILE_SIZE, i * TILE_SIZE, TILE_SIZE, TILE_SIZE, omp_get_thread_num()))
          change=1;
...
#pragma omp single
    ...
    for (int j = GRAIN - 1; j >= 0; j--)
      ...
      #pragma omp task firstprivate(i,j) shared(change) depend(out:dep[i][j]) depend(in: dep[i][j-1],dep[i][j-1])
        if(change |= tile_up_left (j * TILE_SIZE, i * TILE_SIZE, TILE_SIZE,TILE_SIZE, omp_get_thread_num()))
        change=1;
