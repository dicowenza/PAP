...
#pragma omp parallel for collapse(2) //schedule(dynamic) 
    for (int y = 0; y < DIM; y += TILE_SIZE){
      for (int x = 0; x < DIM; x += TILE_SIZE){
        if((x==0)||(x==DIM-TILE_SIZE)||(y== DIM-TILE_SIZE)||(y==0))
          do_tile(x, y, TILE_SIZE, TILE_SIZE, omp_get_thread_num());//sur les bords
        else  
          do_inner_tile(x, y, TILE_SIZE, TILE_SIZE, omp_get_thread_num());
      }
    }
    swap_images ();
  }
